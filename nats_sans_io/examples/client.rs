...
async fn main() {
    ...
    let mut binding = NatsBinding::new(Timeouts {
        ...
        pong_delay: Duration::from_secs(1),
        pong_interval: Duration::from_secs(30),
    });

    log::info!("Starting event loop");

    let mut send_ping_ticker = tokio::time::interval(Duration::from_secs(10));
    let mut recv_ping_ticker = tokio::time::interval(Duration::from_secs(10));
    let mut recv_pong_ticker = tokio::time::interval(Duration::from_secs(10));
    ...
    loop {
        ...
        tokio::select! {
            ...
            time = recv_ping_ticker.tick() => {
                let _ = binding.handle_recv_ping_timeout(time.into());
            }
        ...
        if let Some(tick) = binding.poll_send_ping_timeout() {
        ...
        if let Some(tick) = binding.poll_recv_ping_timeout() {
